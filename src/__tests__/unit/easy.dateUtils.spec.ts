...
describe('getEventsForDay', () => {
  it('특정 날짜(20일)에 해당하는 이벤트만 정확히 반환한다', () => {
    const eventList = getEventsForDay(events as Event[], 20);
    expect(eventList.every((event) => event.date === '2025-02-20')).toBe(true);
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    const eventList = getEventsForDay(events as Event[], 10);
    expect(eventList).toEqual([]);
  });

  it('날짜가 0일 경우 빈 배열을 반환한다', () => {
    const eventList = getEventsForDay(events as Event[], 0);
    expect(eventList).toEqual([]);
  });

  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    const eventList = getEventsForDay(events as Event[], 32);
    expect(eventList).toEqual([]);
  });
});
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2025-02-14'));
    expect(formattedWeek).toBe('2025년 2월 2주');
  });

  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2025-02-02'));
    expect(formattedWeek).toBe('2025년 2월 1주');
  });

  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2025-02-28'));
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    const formattedDate = formatDate(new Date('2025-02-03'));
    expect(formattedDate).toBe('2025-02-03');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    const formattedDate = formatDate(new Date('2025-02-03'), 13);
    expect(formattedDate).toBe('2025-02-13');
  });

  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const formattedDate = formatDate(new Date(2025, 1, 3));
    expect(formattedDate).toBe('2025-02-03');
  });
